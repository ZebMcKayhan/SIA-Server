...
In addition to the plaintext protocol described below, the Galaxy panel can wrap the entire SIA event sequence in a proprietary encryption layer. When enabled, the panel initiates this layer with a four-step cryptographic handshake *before* sending the first `ACCOUNT_ID` block.

**Handshake Overview:**
1.  **Panel → Server:** Sends a static `StartEnc` message to request an encrypted session.
2.  **Server → Panel:** Responds with a temporary `ServerKey` (an RSA-1024 public key).
3.  **Panel → Server:** Sends back a `PanelKey` containing the AES session key, encrypted with the server's public key.
4.  **Server → Panel:** Sends a final `KeyAck` to confirm.

After this handshake, all subsequent SIA blocks (Account ID, New Event, etc.) and the server's ACKs are encrypted using AES-128-ECB. The underlying SIA message framing remains similar but has a pre-encryption length prefix.

Detecting an encrypted session is done by checking the first 5 bytes of the connection for the static `StartEnc` message (`05 01 81 59 68`).

> **For a complete, in-depth analysis of the proprietary cryptography**, including implementation details for the RSA key exchange (`e=3`, little-endian format), AES key extraction, internal validation, and the full proof-of-concept script, please refer to the dedicated document:
> **[Honeywell Galaxy Flex — Encrypted ARC Protocol](./Encrypted_Protocol.md)** 

### Message Block Framing (Plaintext)
...
-   **Checksum Byte:** A single byte used for integrity checking.
    ...
    -   *Formula:* `Checksum = 0xFF ^ (Length Byte) ^ Command Byte ^ (all bytes in Payload)`
