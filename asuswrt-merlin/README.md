...
```bash
opkg update
opkg install python3 python3-requests python3-uvloop
```

For optional encryption support, also install:
...
You can now start the service manually. This will launch `sia-server.py` and set up the monitoring cron job.
...
-   **Reload configuration (accounts and log level):**
    ```bash
    kill -HUP $(ps | grep "[s]ia-server.py" | awk '{print $1}')
    ```
...
2.  **Dependencies:** You must be able to install the `requests` library via your platform's package manager (e.g., `opkg` on OpenWrt). For optional encryption support, also install `python3-cryptodome` (the package name may vary by platform).
3.  **Service Management:** You will need to create your own service/init script that is compatible with your system's init process.
    ...
    -   You will need to ensure the script starts `sia-server.py`. The IP Check Service, if enabled, runs as an integrated part of `sia-server.py` and does not need to be started separately.
